...
1. **Lead with a tension** — the first paragraph is wrapped in
   `<p class="lead"> … </p>`. It gets a drop cap, so open with a vivid, specific
   framing of the problem or the surprise. No throat-clearing, no "In this essay".

2. **2–5 `##` sections**, each a real step in the thinking. Headings are short and
   declarative ("The agent's keyboard: scripting, not clicking"). They render with
   an accent rule, so make them earn it.
...
- `<p class="caption">…</p>` under a figure.
